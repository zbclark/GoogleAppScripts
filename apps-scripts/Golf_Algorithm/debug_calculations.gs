...
function createDebugCalculationSheet(ss, processedData, metricGroups, groupStats) {
  ...
  let debugSheet = ss.getSheetByName("🔧 Debug - Calculations");
  if (debugSheet) {
    debugSheet.clear();
  } else {
    debugSheet = ss.insertSheet("🔧 Debug - Calculations");
  }
  ...
  try {
    ...
    console.log("Sheet already visible or error showing sheet");
  }
