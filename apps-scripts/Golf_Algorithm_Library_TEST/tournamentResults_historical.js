...
/**
 * SANDBOX MODE - Fetches historical tournament data to a separate test sheet
 * Safe for testing - does NOT modify Historical Data sheet
 */
function fetchHistoricalTournamentResultsSandbox() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  
  // Create or get sandbox sheet
  let resultsSheet = ss.getSheetByName("Tournament Results - Sandbox");
  if (!resultsSheet) {
    resultsSheet = ss.insertSheet("Tournament Results - Sandbox");
    resultsSheet.getRange("A1:Z1")
      .setBackground("#FFF3CD")
      .setFontWeight("bold");
    resultsSheet.getRange("A1").setValue("🧪 SANDBOX MODE - Safe Testing Environment");
  }
  
  // Call the main function but with sandbox sheet and skip Historical Data writes
  return fetchHistoricalTournamentResultsImpl(resultsSheet, true);
}

/**
 * PRODUCTION MODE - Fetches historical tournament data
