...
function runCompleteModelAnalysisWithYearPrompt() {
  const selectedYear = configureGolfDataYear();
  if (!selectedYear) {
    return;
  }
...
function runValidation2025() {
  setGolfDataYear(2025);
  writeValidationToSheet(2025);
}

function runValidation2026() {
  setGolfDataYear(2026);
  writeValidationToSheet(2026);
}

function runValidationWithYearPrompt() {
  const selectedYear = configureGolfDataYear();
  if (!selectedYear) {
    return;
  }
  writeValidationToSheet(selectedYear);
}

function writeValidationToSheet(year) {
  try {
    const resolvedYear = year ? String(year) : getGolfDataYear();
    ...
    const sheetName = `${resolvedYear} Validation`;
