...
async function updateHistoricalDataFromButton() {
  ...
  try {
    ...
    // 2. Get period selection and calculate date range
    const period = configSheet.getRange("F13").getValue();
    const dateRange = calculateDateRange(period);
    if (!dateRange) throw new Error("Failed to calculate date range");
    const { startDate, endDate } = dateRange;

    // 2.5 Get special event IDs from Configuration G32:G37
    const specialEventRanges = [
      configSheet.getRange("G29"),        // Single cell
      configSheet.getRange("G33:G37"),     // Driving/Iron range
    ...
    logDebug(`Found ${uniqueSpecialEventIds.length} special events (including putting surfaces): ${uniqueSpecialEventIds.join(', ')}`);

    // 3. Get event IDs within date range
    ...
    tournamentData.forEach(row => {
      ...
      if (isNaN(eventDate.getTime())) {
        logDebug(`Skipping invalid date for event ${eventId}: ${row[1]}`);
        return;
      }
      ...
      if (existingEvents.has(eventKey)) {
        logDebug(`Skipping duplicate event: ${eventKey} `);
        return;
      }
      ...
      if (isSpecialEvent) {
        logDebug(`Adding special event: ${eventKey} for tour ${tour}`);
      ...
      const isValidRegularEvent = 
        ...
        eventTime <= endDate.getTime();

      if (isValidRegularEvent) {
        logDebug(`Adding regular event: ${eventKey}`);
        filteredEvents.push({ tour, eventId, year });
        existingEvents.add(eventKey);
      } else {
