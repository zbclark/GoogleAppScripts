...
function loadCsv(filePath, options = {}) {
    ...
    // Print first 10 lines of raw CSV for diagnostics
    console.log(`\nFirst 10 lines of raw ${path.basename(filePath)}:`);
    lines.slice(0, 10).forEach((line, idx) => console.log(`Line ${idx}:`, line));
  let headerLineIdx = 4;
  ...
  if (processedLines.length > 0) {
    console.log(`Processed header line for ${path.basename(filePath)}:`);
