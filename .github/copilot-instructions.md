...
The purpose of the modelOptimizer and validation library is to optimize the model's performance in predicting tournament outcomes and to validate the accuracy of those predictions against actual results - as if I were betting on the outcomes

For now, we need to maintain parity between the `modelCore.js` file and `results.js` file, but we should eventually consolidate the logic into a single source of truth to avoid discrepancies and ensure consistency across the project.  for now the `results.js` file is the source of truth.  
